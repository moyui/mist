...
- 证据：compose.yaml 4 处 `OO_OTLP_AUTH_BASE64:-<OO_AUTH_BASE64_REDACTED>`（= base64("<OO_USER_REDACTED>:<OO_PASSWORD_REDACTED>") 可逆解）；`.env.example:66` 同值；`deploy-defaults.ps1` 无该变量（部署链未显式管理）
