...
const sourceRoot = join(repositoryRoot, 'libs', 'chancore', 'src');
...
describe('ChanCore pure boundary', () => {
  it('is registered as one exact Nest library alias', () => {
    ...
    expect(nestConfig.projects.chancore).toEqual({
      type: 'library',
      root: 'libs/chancore',
      entryFile: 'index',
      sourceRoot: 'libs/chancore/src',
      compilerOptions: {
        tsConfigPath: 'libs/chancore/tsconfig.lib.json',
      },
    });
    expect(tsConfig.compilerOptions.paths['@app/chancore']).toEqual([
      'libs/chancore/src/index.ts',
    ]);
    expect(tsConfig.compilerOptions.paths['@app/chancore/*']).toBeUndefined();
    expect(packageJson.jest.moduleNameMapper['^@app/chancore$']).toBe(
      '<rootDir>/libs/chancore/src/index.ts',
    );
  });
