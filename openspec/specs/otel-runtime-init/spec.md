# otel-runtime-init Specification

## Purpose
Define the single OTel SDK initialization path through otel-preload.js: no misleading fallback when run without -r preload, and no dead serviceName parameter (name comes only from OTEL_SERVICE_NAME).
## Requirements
