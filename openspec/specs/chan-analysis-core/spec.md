...
## Purpose
Define the pure `@app/chancore` library as the single stateless Chan analysis boundary: deterministic
merge-K, Fenxing, Bi, Channel, Duan, Duan-level Channel and divergence outputs from approved in-memory K
input, with a versioned algorithm facade and no persistence or external I/O.
## Requirements
