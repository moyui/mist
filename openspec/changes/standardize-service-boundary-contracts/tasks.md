...
- [x] 1.1 记录 `mist`、`mist-fe`、`mist-skills` branch、HEAD、dirty/worktree 和 active changes。
- [x] 1.2 建立 controller → interceptor/filter → HTTP wire → frontend/skills 与
  HTTP request → RPC sender → handler → result → HTTP adapter 影响链。
- [x] 1.3 记录现有成功 200/201、expected business rejection、validation error、structured
  HttpException、unknown exception、requestId 和 Chan app contract tests。
- [x] 1.4 审计 OpenAPI DTO、frontend unwrap、skills MistClient 对 `success/statusCode/message` 的真实
  依赖，以及新增必填 `code` 和 HTTP-200 business rejection 的迁移影响。

## 2. Shared Transport Library

- [x] 2.1 创建 Nest library `transport`、`libs/transport`、`tsconfig.lib.json` 和 Nest CLI project；
  精确导出 `@app/transport/http` 与 `@app/transport/rpc`，不得增加根 alias、wildcard alias、相对路径
  或 deep import 绕过 subpath barrel。
- [x] 2.2 实现 `http/index.ts` 与 `rpc/index.ts` 的分层 public API，并迁移
  `ApiResponseDto`/`ApiErrorDto`；删除重复 interface/DTO 字段定义并保留 Swagger metadata。
- [x] 2.3 迁移 response interceptor、exception filter、response-message/OpenAPI decorator、request
...
- [x] 2.4 将 `apps/mist` 与 `apps/chan` 切换到 shared HTTP implementation，删除 `apps/chan` 对
...
- [x] 3.1 使用真实 HTTP response status 填充成功 envelope `statusCode`，覆盖 200/201/202、
  `undefined -> data:null` 和 204 无 body behavior，且不保留 body-200 compatibility branch。
- [x] 3.2 实现 `@HttpResponseMessage()`，默认 `SUCCESS`，并证明业务 `data.message` 不会被自动提升。
- [x] 3.3 实现 `ApiErrorDto<TCode,TData>` 的必填 stable `code` 与可选 typed `data`；`code` 用于机器
...
- [x] 3.3.1 对 public HTTP code 实施 `^[A-Z][A-Z0-9_]{0,63}$` runtime validation；覆盖空值、数字、
...
- [x] 3.4 实现 generic `HttpBusinessRejection<TCode,TData>` class 与 `instanceof` interceptor
...
- [x] 3.5 在 HTTP 入口生成单一服务器侧 `requestId`，成功、失败和日志复用；拒绝未经验证的客户端
...
- [x] 3.6 更新 Swagger/OpenAPI、interceptor/filter/integration tests 和 Chan HTTP contract tests；
...
- [x] 3.7 让 HTTP filter 成为同步请求最终 authoritative log owner：4xx/429 warning 无 stack，
...
- [x] 3.8 实现 recursive validation-error flattening，以稳定 dotted path（含数组下标）合并父子
...
- [x] 3.9 实现 `ApiEnvelopeResponse` 与 `ApiTechnicalErrorResponse`：使用 allOf 引用 domain data，
...
- [x] 4.2 实现 strict contract tests：缺失/空 correlation、both/neither result branch、未知字段、
  ...
  响应或日志。
- [x] 4.2.1 覆盖 HTTP 并行 RPC 复用 request correlation、result mismatch fail-closed、error channel
...
- [x] 4.3 固定 `domain.resource.action.vN` 命名及 consumer-first 升级规则；V1 payload 不重复
...
- [x] 4.4 证明 HTTP-only 字段、业务 payload/error code、idempotency、retry 和 arbitrary details
  未进入 shared RPC envelope。
- [x] 4.5 添加与 Nest common/core 对齐的 `@nestjs/microservices@10.4.15`，实现
...
- [x] 4.6 添加精确 `@app/backtest`、`@app/signal`、`@app/strategy` root-barrel boundary tests：禁止
...
- [x] 5.1 只读审计 `mist-fe`、`mist-skills` 对现有 endpoint 的兼容性并记录风险；不得修改 consumer
  代码或 tests，严格解析迁移由 `harden-http-envelope-consumers` owning change 交付。
- [x] 5.2 运行 `mist` lint/typecheck/test/contract/build 完整基线；本 change 不执行或声称
  frontend/skills 修改基线。
- [x] 5.3 执行本 change、相关 active changes 与 stable specs strict validation 和
  `git diff --check`。
- [x] 5.4 全仓检索旧 response interface/DTO、body `statusCode: 200`、重复 requestId generator、
...
- [x] 5.5 记录自动化通过、环境阻塞和未执行项；本 change 不需要交易终端 HIL。
