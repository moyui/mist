---
name: candle-pipeline-attribution-gaps
version: 0.1.0
---

# Candle Pipeline Attribution Gaps

## ADDED Requirements

### Requirement: R1: 计数 source+symbol 归因
skip/discard 计数指标 SHALL 携带 `source` 与 `securityId`/`symbol` label，支持按源与标的
归因；reason/source 枚举 SHALL 保持有界；label 基数 SHALL 受护栏约束（超阈值回退
source-only 或告警）。
...
#### Scenario: 单标的可归因
- Given 同一 source 下多个标的在 skip
- When 按 securityId/symbol 过滤查询
- Then 每个标的 SHALL 有独立计数可归因

### Requirement: R2: finalize span 判定结果
`candle.due.finalize` span SHALL 携带判定结果 attribute：`verdict`（sealed/discarded）、
discarded 时 `discardReason`。（vwap 一致性检查在 deploy workflow 层
`read-windows-realtime-candle-closed` 的 `vwapClassification` 提供，不属于 span。）
