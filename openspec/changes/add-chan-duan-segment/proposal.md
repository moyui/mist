...
- `ChanCore` 新增 `createDuan(orderedK)`，**返回结果对齐笔**：返回 `ChanDuanTwoPhaseResult { phaseA, phaseB }`
  （≡ `createBi` 的 `ChanBiTwoPhaseResult`）；`ChanDuan` 镜像 `ChanBi` 字段（端点从分型升为笔、构成单元从 K
  升为笔）。对齐的是返回结果，不是算法内容。
- 段算法 = **标准特征序列法**（缠中说禅第67课原典），**不采用现有两阶段 / `mergeSpans` 范式**——
  特征序列法是"单遍递推 + 包含处理 + 分型 + 缺口两种情况（第二种带回溯倒推）"，与笔的栈式归约同类，
  不是中枢的"枚举候选 + 不动点合并"。返回的 `phaseA/phaseB` 语义：phaseA = 特征序列分型候选段，
  phaseB = 经缺口两种情况（含第二种回溯）确认的最终段。
...
- 新增 library-owned 类型：`DuanType`、`DuanStatus`、`ChanDuan`（镜像 `ChanBi`）、`ChanDuanTwoPhaseResult`
  （≡ `ChanBiTwoPhaseResult`）。特征序列/分型为算法内部中间态，不单独暴露（≡ 笔的 `findFenxings` 是独立
  facade，不塞进 `createBi` 结果）。
- 新增 HTTP 端点 `POST /v1/chan/duan`，返回 `{ phaseA, phaseB }`（≡ `/v1/chan/bi`），递归遵守现有 `high/low` 契约。
