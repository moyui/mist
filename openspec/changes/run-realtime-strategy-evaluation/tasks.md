...
- [ ] 1.3 建立 sealed candle → handoff → context → analysis → evaluator → episode → transaction →
...
- [ ] 2.4 实现 quantity plan 的当日 pre-anchor projector seed、current observation exactly-once 和
...
- [ ] 3.2 实现 candle commit 后非阻塞 `sealed_bar` handoff、确定性 jobId、最小 trigger payload 和
  attempts/retention/stalled/deadline 配置；queue failure 不回滚 candle。
...
- [ ] 4.1 新建单实例 Hybrid Nest project `signal`、`apps/signal`、`SignalAppModule`、HTTP health、TCP
  registry-refresh 和 BullMQ worker，共享唯一 registry/window/analysis/episode state owner。
...
- [ ] 4.5 用 negative tests 证明 V1 只接受 `sealed_bar`；snapshot/raw provisional input 不进入
