...
- [ ] 1.1 `[datasource]` 实现共享 StallDetector 三态状态机（IDLE/PUSHING/
      VERIFIED + escalated，活动信号注入，fake clock 可测，无 I/O）（D1）。
- [ ] 1.2 `[datasource]` 活动窗口：读 `MIST_ACTIVITY_WINDOWS`（默认
      `09:15-11:30,13:00-15:00` UTC+8）判定当前是否窗口内；窗口外强制 IDLE
      （零重连零检出零告警）（D1/D5）。
- [ ] 1.3 `[datasource]` 共享 metric：`mist_datasource_subscription_stall_active
...
- [ ] 1.4 `[datasource tests]` StallDetector 单测：三态转换、时段外 IDLE、
      午休/收盘不误报、盘前断流检出、escalated 边界、恢复清除（fake clock）
      （D1/D5）。

## 2. TDX datasource（mist-datasource 仓）

- [ ] 2.1 `[datasource]` gateway 接入活动信号：observability 帧 callback_count
      进展喂入（新增 `observe_bridge_activity` 入口，现在只打日志）+ 快照接收
      更新 lastActivity（D1/D2）。
- [ ] 2.2 `[datasource]` gateway 维护状态机实例 + asyncio watchdog（tick 默认
      5s）；**poll 语义按状态切换**：PUSHING 返回全量 subscribe、VERIFIED/IDLE
      返回 diff（D2）。
- [ ] 2.3 `[datasource]` health 暴露 `pushState/stallDetected/stallEscalated`
      + metric + 日志（D1/D6）。
- [ ] 2.4 `[datasource]` `register_owner` 记录 generation 转换日志
      （old→new + 时间戳 + owner_id）+ `owner_registration_total` 递增（D6）。
- [ ] 2.5 `[datasource tests]` gateway 状态机单测：PUSHING→poll 全量、
      VERIFIED→poll diff、快照恢复切 VERIFIED、静默切 PUSHING、夜间/时段外
      不误报（fake clock + fake observability 帧）（D2）。
- [ ] 2.6 `[datasource tests]` register_owner 重置→poll 全量 subscribe 回归
      测试（现有隐式行为补测试，D2 佐证）。

## 3. QMT datasource（mist-datasource 仓）

- [ ] 3.1 `[datasource]` controller 接入活动信号：`_callback_last_seen` +
      lastQuoteAt → StallDetector + asyncio watchdog（tick 默认 5s）（D1/D3）。
- [ ] 3.2 `[datasource]` **sync 语义按状态切换（消除 QMT 每 60s 无条件循环
...
- [ ] 3.3 `[datasource]` health 暴露 `callbackLastSeenAgeSeconds` +
      `pushState/stallDetected/stallEscalated`（D6）。
- [ ] 3.4 `[datasource]` `qmt/main.py` 注册 snapshot_age gauge（修复与 TDX
      不对称）（D6）。
- [ ] 3.5 `[datasource]` QMT gateway generation 转换日志 + `owner_registration_total`
      递增（owner 变化时）（D6）。
- [ ] 3.6 `[datasource tests]` controller 状态机 + 强制全量重发单测：stall 检出
      立即重发、VERIFIED 零 SDK 调用、与 journal/reconciliation 互斥、
      e04a1c8 absent 语义兼容（D3/D5）。
- [ ] 3.7 `[datasource tests]` health 字段与 snapshot_age gauge 注册单测。

## 4. TDX 桥脚本 v3.1（mist-datasource 仓 tdx/builtin_bridge）

- [ ] 4.1 `[datasource]` 启动上下文日志：pid / 父进程 / 启动时刻 / transport /
      buildId（D6）。
- [ ] 4.2 `[datasource]` re-arm 配置门：`REARM_ENABLED`（默认 false）；翻 true
      时 PUSHING 态"全量重发后 callback_count 无增长"路径执行一轮
      unsubscribe+subscribe 强制重挂（D4）。
- [ ] 4.3 `[datasource]` `bridgeBuildId`/`bridgeArtifactSha256` 更新（v3.1）。
- [ ] 4.4 `[datasource tests]` 桥脚本测试：启动日志断言、re-arm 路径（静态/
...
- [ ] 6.2 `[deploy]` A1 盲区修复：按 source 拆分（A1_tdx/A1_qmt）或 label
      过滤（oo-alerts/rules.json + sync-oo-alerts.ps1）（D7）。
- [ ] 6.3 `[deploy]` 新规则 A7：`stall_active ≥ 1` → P1（双源一条，按 source
      label）（D7）。
- [ ] 6.4 `[deploy]` 活动窗口单点：compose 给 tdx-datasource / qmt-datasource /
      mist-backend（receiver）统一设 `MIST_ACTIVITY_WINDOWS`（默认
      `09:15-11:30,13:00-15:00`），一份定义两处消费，不各自硬编码（D5/D8）。
- [ ] 6.5 `[datasource]` 窗口判定一致性：datasource 容器时区确认（UTC+8 判定
      MIST_ACTIVITY_WINDOWS，必要时显式 TZ），与 receiver 同为窗口语义（D5）。
- [ ] 6.5 `[deploy tests]` test-workflow-config 断言更新（A1 拆分 + A7）。
...
- [ ] 8.1 spec 确认通过后写实施计划（代码级：文件级改动、函数签名、测试
      用例、验证命令）。
- [ ] 8.2 实施计划确认后落地（worktree 分支 + 单测 + 验证 + 合并）。
- [ ] 8.3 `openspec validate realtime-subscription-restart-recovery --strict`
