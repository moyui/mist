...
- [ ] 1.1 `[mist]` 在 `apps/chan` 或 `apps/mist` 中定义 `ChanProjectionVo` 数据契约（支持 bi, duan, zhongshu, signals 平铺导出）。
- [ ] 1.2 `[mist]` 实现 `GET /v1/chan/projection` 控制器，聚合 Chancore 缠论结构为直接可渲染的平铺数组。
- [ ] 1.3 `[mist]` 编写 `projection.controller.spec.ts` 契约与单元测试（门禁保证延迟 `< 50ms`）。

## 2. QMT 终端原生绘图集成 (QMT Client - Phase 1 重点)

- [ ] 2.1 `[mist-datasource]` 编写 QMT 原生 Python 主图指标脚本 `MistChan.py`。
