...
- [ ] 1.2 `[mist]` `degradedRuntimeReasons` 改为事件性判定：
  ...
  保持累计判定不变；`queue_overflow` 由 snapshot/dueAdmission 两个时间戳取最近者驱动。
- [ ] 1.3 `[mist]` **确定性拒绝与意外失败分离**：finalizer/due 注册的过期交易日等
  确定性拒绝路径不再驱动 degraded（仅累积计数）；仅意外运行时错误更新时间戳。
  实现选型以 owner 过审结论为准（design §3 待评审问题 1）。
- [ ] 1.4 `[mist]` 累计计数器保留用于监控（总量语义不变），observation type 增加
  lastFailureAtMs 字段，且窗口参数经 `libs/config` 校验（默认 300000，范围 60000..900000）。

## 2. 观测与监控同步

- [ ] 2.1 `[mist tests]` 单测：事件性失败在窗口内 degraded、窗口外恢复、窗口内再次失败
  刷新时间戳、持续状态不受窗口影响、确定性拒绝不降级、queue 双计数器各自
  时间戳取最近者、counter 保留累计。
