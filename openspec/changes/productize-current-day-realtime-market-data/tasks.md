...
- [ ] 2.8 在 `RealtimeSnapshotIngressService.handleSnapshot()` transport acceptance 后同步记录 `acceptedAt` 并立即提交到无第三方依赖的 `Map<securityId, Promise<void>>` keyed queue；snapshot update与finalizer共用该队列，同 security 串行、不同 security 并行 I/O、task 执行前复核 epoch，前序失败不阻断后续。
