...
- [ ] 1.2 向项目负责人评审 canonical decimal grammar、precision/scale、TDX numeric rejection 和 QMT provider-float provenance。
...
- [ ] 1.3 把接受的 quantity 规则写回 design/specs；未确认前不修改 producer、decoder 或 fixture。

## 2. Candle 语义评审门禁

- [ ] 2.1 提交 A 股 bucket/session、baseline/delta、counter reset、乱序、午休、跨日和 restart 状态表。
...
- [ ] 2.2 提交 valid/discarded、grace、hard horizon、due failure 和 shutdown 语义供项目负责人逐项确认。
- [ ] 2.3 提交 Node queue/memory、Redis record/retention/command/capacity limits 候选值与测量方式并确认。
...
- [ ] 2.4 将全部接受结论写回 design/specs 后，才开始 candle 实现。
...
- [ ] 4.1 实现可注入 Clock、session bucket resolver 和 per-security bounded execution chain。
- [ ] 4.2 实现 open candle aggregator、volume/amount 独立 baseline/delta/reset、同日 null-counter hold、
  无 baseline 的 sealed null、乱序和 capacity fail-closed；覆盖 candle owner 不得跨日继承 cumulative
  baseline，也不得在 sealing 时用前一根 K 覆盖 raw null。
- [ ] 4.3 实现 due registration/scanner、valid/discarded finalizer 和 atomic Redis state transition。
- [ ] 4.4 实现 bounded manifest replay、上海 D+1 00:00 exact-key expiry、Node trading-day rollover 和
  restart/open-state diagnostics；证明 prior-day state 不被读取且 BullMQ namespace 不受影响。
- [ ] 4.5 将可失败隔离的 candle sink 接到 latest-memory acceptance 之后，证明无策略或 MySQL 副作用。

## 5. 部署、监控与验收

- [ ] 5.1 完成 market Redis Compose/env/health/startup/rollback contract，模式保持 off。
