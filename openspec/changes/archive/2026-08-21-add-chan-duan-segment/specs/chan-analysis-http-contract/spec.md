...
- **WHEN** merge-K, Fenxing, Bi, Channel or Duan data is returned by `/v1/chan/*`
- **THEN** every price interval owned by K, merged K, Fenxing, Bi, Duan or Channel MUST use `high` and `low`
- **AND** the same rule MUST apply recursively to `mergedData`, `originData`, `startFenxing`, `endFenxing`,
  Channel `bis` and Duan `originBis`
